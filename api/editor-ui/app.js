...
*/
/* dig.D Vídeo — interface adaptada pela dig.D a partir do edvid (MIT, © 2026 Creator Factory).
 * Licença preservada em LICENSE-edvid. Linha do tempo interativa.
 * Adapted app: everything per-session comes from /api/state (state.json,
...
// ---------- dom ----------
// O iframe recebe o tema e o modo de apresentação pela URL.
const editorParams = new URLSearchParams(window.location.search);
document.documentElement.classList.toggle('dark', editorParams.get('tema') === 'escuro');
document.documentElement.classList.toggle('embutido', editorParams.get('embutido') === '1');

// O tema da URL já foi aplicado. Lê a paleta uma vez, fora do desenho.
const canvasColors = (() => {
  const css = getComputedStyle(document.documentElement);
  return Object.freeze({
    rulerText: css.getPropertyValue('--canvas-ruler-text').trim(),
    rulerTick: css.getPropertyValue('--canvas-ruler-tick').trim(),
    waveLine: css.getPropertyValue('--canvas-wave-line').trim(),
    waveFill: css.getPropertyValue('--canvas-wave-fill').trim(),
  });
})();
...
const STYLE_CATALOG = {
  edits: [
    {
      ...
      mock: `<svg viewBox="0 0 66 118" xmlns="http://www.w3.org/2000/svg">
        <rect x=".5" y=".5" width="65" height="117" rx="7" fill="var(--bg0)" stroke="var(--stroke)"/>
        <rect x="3" y="3" width="60" height="112" rx="5" fill="var(--glass-strong)"/>
        <circle cx="33" cy="48" r="13" fill="var(--faint)"/>
        <path d="M12 115a21 21 0 0142 0z" fill="var(--faint)"/>
        <rect x="14" y="14" width="38" height="4.4" rx="2.2" fill="var(--dim)"/>
        <rect x="20" y="21.5" width="26" height="4.4" rx="2.2" fill="var(--faint)"/>
        <rect x="12" y="74" width="42" height="11" rx="5.5" fill="var(--bg0)" stroke="var(--orange)"/>
        <rect x="16" y="78.5" width="12" height="2.4" rx="1.2" fill="var(--orange)"/>
        <rect x="30" y="78.5" width="8" height="2.4" rx="1.2" fill="var(--dim)"/>
        <rect x="40" y="78.5" width="10" height="2.4" rx="1.2" fill="var(--dim)"/>
      </svg>`,
    },
    {
      id: 'split',
      ...
      mock: `<svg viewBox="0 0 66 118" xmlns="http://www.w3.org/2000/svg">
        <rect x=".5" y=".5" width="65" height="117" rx="7" fill="var(--bg0)" stroke="var(--stroke)"/>
        <rect x="3" y="3" width="60" height="36" rx="5" fill="var(--pink-soft)"/>
        <circle cx="17" cy="15" r="3.6" fill="var(--orange)"/>
        <path d="M6 36l11-11a2 2 0 013 0l7 7 5-4a2 2 0 013 0l11 8" fill="none" stroke="var(--orange)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M3 40.5h60" stroke="var(--dim)" stroke-width="1.2"/>
        <rect x="3" y="42" width="60" height="73" rx="5" fill="var(--glass-strong)"/>
        <circle cx="33" cy="70" r="12" fill="var(--faint)"/>
        <path d="M15 115a18 18 0 0136 0z" fill="var(--faint)"/>
        <rect x="12" y="35" width="42" height="11" rx="5.5" fill="var(--bg0)" stroke="var(--orange)"/>
        <rect x="16" y="39.5" width="12" height="2.4" rx="1.2" fill="var(--orange)"/>
        <rect x="30" y="39.5" width="8" height="2.4" rx="1.2" fill="var(--dim)"/>
        <rect x="40" y="39.5" width="10" height="2.4" rx="1.2" fill="var(--dim)"/>
      </svg>`,
    },
    {
      id: 'split2',
      ...
      mock: `<svg viewBox="0 0 66 118" xmlns="http://www.w3.org/2000/svg">
        <rect x=".5" y=".5" width="65" height="117" rx="7" fill="var(--bg0)" stroke="var(--stroke)"/>
        <rect x="3" y="3" width="60" height="65" rx="5" fill="var(--glass-strong)"/>
        <circle cx="33" cy="24" r="11" fill="var(--faint)"/>
        <path d="M16 68a17 17 0 0134 0z" fill="var(--faint)"/>
        <path d="M3 69.5h60" stroke="var(--dim)" stroke-width="1.2"/>
        <rect x="3" y="71" width="60" height="44" rx="5" fill="var(--pink-soft)"/>
        <circle cx="17" cy="83" r="3.6" fill="var(--orange)"/>
        <path d="M6 111l11-11a2 2 0 013 0l7 7 5-4a2 2 0 013 0l11 8" fill="none" stroke="var(--orange)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        <rect x="12" y="58" width="42" height="11" rx="5.5" fill="var(--bg0)" stroke="var(--orange)"/>
        <rect x="16" y="62.5" width="12" height="2.4" rx="1.2" fill="var(--orange)"/>
        <rect x="30" y="62.5" width="8" height="2.4" rx="1.2" fill="var(--dim)"/>
        <rect x="40" y="62.5" width="10" height="2.4" rx="1.2" fill="var(--dim)"/>
      </svg>`,
    },
  ],
  ...
  elements: [
    {
      id: 'tracking',
      name: 'Câmera acompanha a pessoa',
      def: false,
    ...
    {
      id: 'zoomCuts',
      name: 'Aproxima e afasta nos cortes',
      def: true,
...
const NONE_MARK = '<svg viewBox="0 0 64 34" style="width:64px;height:34px">'
  ...
  + 'stroke="var(--stroke)" stroke-width="1.5" stroke-dasharray="4 3"/>'
  + '<path d="M14 24L50 10" stroke="var(--dim)" stroke-width="1.5" '
...
function buildStackedDemo(host) {
  ...
  const shadow = `drop-shadow(0 ${(5 * s).toFixed(2)}px ${(9 * s).toFixed(2)}px var(--shadow-preview))`;
  const all = [];
  let idx = 0;
 ...
 */
const HEADLINE_TEXT = 'É assim que vai ficar o seu título';
const HL_MIN = 28;
const HL_STYLES = {
...
function buildHeadlineDemo(host, styleId) {
  ...
  if (styleId === 'outline') {
    box.style.webkitTextStroke = `${7 * s}px var(--media-black)`;
  }
  lines.forEach((l, i) => {
    if (!l) return;
    const d = el('div', '', box);
    ...
    if (styleId === 'misto') d.style.color = i === 1 ? 'var(--hl-accent)' : 'var(--media-white)';
    d.textContent = l;
  });
}
...
const LABEL_W = 144; // .track-label width (content x offset of lanes)
const MIN_SEG = 0.2; // s
let THUMB_EVERY = 2.0;
...
function refreshHeader() {
  ...
  // A PILULA e do momento: "acabei de salvar". O BOTAO e do estado do
  // arquivo: existe edicao salva esperando, tenha sido agora ou anteontem.
  // Se dependesse da sessao, fechar a aba escondia o unico jeito de aplicar.
  const podeAplicar = S.hasPending && n === 0;
  $('savedPill').classList.toggle('hidden', !(S.savedPending && n === 0));
  // Dentro do dashboard quem aplica e o botao de la; aqui ele seria um segundo
  // botao dizendo a mesma coisa. Em tela cheia nao ha dash — e sem ele a tela
  // mandava apertar um botao que nao existia, que foi o que ela encontrou.
  ...
  try {
    ...
    if (sig !== S.lastSig) {
      ...
      if (!hadEdits) {
        ...
        toast('Nova versão do vídeo disponível — salve ou descarte seus ajustes para atualizar', 4000);
      }
    }
...
async function applyState(data) {
  ...
  // Pendências de outra abertura não são uma confirmação de salvamento local.
  S.savedPending = S.savedPending && !!data.hasPendingEdits;
  S.hasPending = !!data.hasPendingEdits;
  S.videoId = data.videoId || S.videoId;

  $('projectName').textContent = S.state.project || '';
  ...
  if (S.workflowVideo !== S.videoId) {
    S.workflowVideo = S.videoId;
    const g = readWorkflowStyle();
    S.style = { ...defaultStyle(), ...(S.state.style || {}), ...g.picks };
    for (const [id, key] of Object.entries({captionScale: 'escala', captionRise: 'subir', headlineScale: 'headlineEscala', headlineLines: 'headlineLinhas', closingWord: 'fechoPalavra', closingBefore: 'fechoAntes', closingAfter: 'fechoDepois', soundtrack: 'trilha'})) {
      if (g[key] != null) $(id).value = g[key];
    }
    if (typeof g.trilhaLigada === 'boolean') $('soundtrackEnabled').checked = g.trilhaLigada;
  }
  S.style.elements = { ...defaultStyle().elements, ...S.style.elements };
...
function buildInsertsDraft() {
  ...
  if (d.hook && d.hook.enabled) {
    list.push({ kind: 'hook', label: `ABERTURA — ${(d.hook.lines || []).join(' / ')}`, start: 0, end: d.hook.endSec || 4 });
  }
  ...
  (d.behind || []).forEach((b, i) => {
    list.push({ kind: 'behind', label: `AO FUNDO ${b.kind === 'words' ? (b.words || []).map((w) => w.t).join(' ') : (b.src || '').split('/').pop()}`, start: +b.start, end: +b.start + +b.dur, ref: i });
  });
...
async function loadThumbsMeta() {
  S.thumbCount = 0;
  try {
    const meta = await videoRequest('/gen/thumbs/meta');
    S.thumbCount = meta.count;
    THUMB_EVERY = meta.interval;
    renderClips();
...
function renderNotes() {
  ...
  for (const n of S.notes) {
    ...
    chip.textContent = `${fmt(n.start)} → ${fmt(n.end)}\n${n.text || '(sem descrição)'}`;
  ...
  $('markText').textContent = S.pendingIn != null ? 'Finalizar trecho' : 'Marcar trecho';
}
...
function toggleMark() {
  ...
  if (S.pendingIn == null) {
    S.pendingIn = t;
    renderNotes();
    toast('Início marcado — avance até o fim do trecho e marque o fim', 2600);
    return;
  }
  const start = Math.min(S.pendingIn, t);
  const end = Math.max(S.pendingIn, t);
  if (end - start < 0.05) {
    toast('Trecho curto demais — avance um pouco mais no vídeo', 2200);
    return;
  }
  S.pendingIn = null;
...
const ACCENT_DEFAULT = getComputedStyle(document.documentElement).getPropertyValue('--hl-accent').trim();
...
function updateAccentNote() {
  const where = [
    ACCENT_USERS.headlines.includes(S.style.headline) && 'no título',
...
function updateSummary() {
  ...
  $('setupSummary').textContent =
    `${styleName('edits', S.style.edit)} · título ${styleName('headlines', S.style.headline)}` +
...
function renderSetup() {
  ...
  const hasVideo = S.videoDuration > 0;
  $('stage').classList.toggle('hidden', show || !hasVideo || typeof S.tab === 'string');
  $('emptyState').classList.toggle('hidden', hasVideo || show || typeof S.tab === 'string');
  ...
  $('setupGo').textContent = 'Guardar escolhas avançadas';
...
$('setupGo').addEventListener('click', async () => {
  if (!S.videoId) { toast('Escolha um vídeo na Fila antes de guardar as escolhas.', 4000); return; }
  try {
  ...
  if (res.ok && (await res.json()).ok) {
    saveWorkflowStyle();
    toast('Escolhas guardadas. Use Aplicar e renderizar para gerar o vídeo com os parâmetros disponíveis.', 6000);
    renderSetup();
    refreshHeader();
  } else {
    toast('Não foi possível enviar. Tente novamente.', 4000);
  }
  } catch (e) { toast('Não foi possível guardar as escolhas. Tente novamente.', 5000); }
});
...
function renderClips() {
  ...
  dl.forEach((r, i) => {
    ...
    if (S.thumbCount > 0 && rl[i]) {
      ...
      for (let k = 0; k < n; k++) {
        const idx = first + k; // Base zero: servidor gera o quadro de n * intervalo segundos.
        if (idx >= S.thumbCount) break;
        const img = el('img', '', strip);
        img.src = `/gen/thumbs/${String(idx).padStart(4, '0')}.jpg?v=${S.mtimes.video || 0}`;
...
function renderJcutAudio() {
  ...
  btn.title = !has ? 'Áudio completo'
    : S.jcutOpen ? 'Áudio completo — ocultar áudio 1 e áudio 2'
                 : 'Áudio completo — mostrar áudio 1 e áudio 2';
  ...
  draftLayout().forEach((r, i) => {
    ...
    if (r.lead > 1e-6) {
      tip += `\nVoz antecipada: ${Math.round(r.lead * S.fps)}f (${Math.round(r.lead * 1000)}ms) `
           + 'de voz antes da imagem';
    }
    if (tf) tip += `\nfinal encurtado em ${tf} quadros`;
    b.title = tip;
  });
}
...
function drawRuler() {
  ...
  ctx.fillStyle = canvasColors.rulerText;
  ctx.strokeStyle = canvasColors.rulerTick;
...
function drawWave() {
  ...
  const mid = h / 2;
  ctx.strokeStyle = canvasColors.waveLine;
  ctx.beginPath(); ctx.moveTo(0, mid); ctx.lineTo(w, mid); ctx.stroke();
  ctx.fillStyle = canvasColors.waveFill;
...
document.addEventListener('keydown', (e) => {
  if (!['1', '2'].includes(String(S.tab)) || ['INPUT', 'TEXTAREA', 'SELECT', 'BUTTON', 'A'].includes(e.target.tagName)) return;
  if (e.key === 'm' || e.key === 'M') {
    e.preventDefault();
    toggleMark();
    return;
  ...
  if (e.key === 'Escape' && S.pendingIn != null) {
    S.pendingIn = null;
    renderNotes();
    toast('Marcação de início cancelada', 1600);
    return;
  }
  if (e.code === 'Space') {
    e.preventDefault();
...
document.querySelectorAll('.tab').forEach((tab) =>
  tab.addEventListener('click', () => {
    ...
    S.tab = ['1', '2'].includes(tab.dataset.tab) ? +tab.dataset.tab : tab.dataset.tab;
    video.pause();
    S.selected = -1;
...
$('btnSave').addEventListener('click', async () => {
  ...
  if ((await res.json()).ok) {
    ...
    toast('Não foi possível salvar. Tente novamente.', 4000);
  }
});
...
$('btnApply').addEventListener('click', async () => {
  if (!S.videoId) { toast('Não sei qual vídeo é este. Recarregue a página.', 5000); return; }
  if (!confirm('Aplicar refaz o corte e apaga as versões com legenda existentes, pois pertenciam ao corte antigo. Continuar?')) return;
  const b = $('btnApply');
  b.disabled = true;
  b.textContent = 'Aplicando…';
  try {
    const r = await fetch(`/api/videos/${S.videoId}/aplicar-edicao`, { method: 'POST' });
    if (!r.ok) {
      const d = await r.json().catch(() => ({}));
      throw new Error(d.detail || 'não consegui aplicar.');
    }
    S.savedPending = false;
    S.hasPending = false;
    toast('Aplicando o corte. O vídeo volta pra fila e refaz sozinho.', 6000);
    refreshHeader();
    openWorkflowTab('fila');
    await fetchVideos();
  } catch (e) {
    toast(e.message || 'não consegui aplicar.', 6000);
  } finally {
    b.disabled = false;
    b.textContent = 'Aplicar o que marquei';
  }
});
...
// Tela dividida: o formulário mantém o rascunho enquanto a fila é consultada.
const splitUI = {video: null, itens: [], loading: false};
function updateSplitPanel() {
  const show = ['split', 'split2'].includes(S.style?.edit);
  $('splitPanel').classList.toggle('hidden', !show);
  if (show && S.videoId && splitUI.video !== S.videoId && !splitUI.loading) loadSplit();
}
async function loadSplit() {
  splitUI.loading = true;
  try {
    const [familias, plano] = await Promise.all([
      videoRequest('/api/arte/familias'), videoRequest(videoPath(S.videoId) + '/split')
    ]);
    $('splitFamily').replaceChildren();
    for (const nome of familias.familias) {
      const option = el('option', '', $('splitFamily')); option.value = nome; option.textContent = nome;
    }
    if (plano.familia) $('splitFamily').value = plano.familia;
    splitUI.itens = plano.itens || [];
    splitUI.video = S.videoId;
    $('splitFraming').value = plano.enquadramento ? JSON.stringify(plano.enquadramento, null, 2) : '';
    if (recursos.arte) $('splitNotice').textContent = plano.enquadramento ? 'Confira as medidas e as artes antes de renderizar.' : 'Sem enquadramento registrado. Informe a medida deste vídeo; não há valor presumido.';
    drawSplitRows();
  } catch (e) { if (recursos.arte) $('splitNotice').textContent = e.message; }
  finally { splitUI.loading = false; }
}
function drawSplitRows() {
  $('splitRows').replaceChildren();
  splitUI.itens.forEach((item, i) => {
    const card = el('article', 'glass workflow-card', $('splitRows'));
    el('h3', 'panel-heading', card).textContent = 'Faixa ' + (i + 1);
    if (item.arquivo) {
      const img = el('img', 'cover-thumb', card);
      img.src = videoPath(S.videoId) + '/split/imagem/' + encodeURIComponent(item.arquivo.split('/').pop());
      img.alt = item.descricao || 'Arte da faixa ' + (i + 1);
    }
    for (const [key, title] of [['inicio', 'Início (segundos)'], ['fim', 'Fim (segundos)'], ['descricao', 'O que desenhar']]) {
      const label = el('label', '', card); label.textContent = title;
      const input = el(key === 'descricao' ? 'textarea' : 'input', '', label);
      if (key !== 'descricao') { input.type = 'number'; input.min = '0'; input.step = '0.1'; }
      input.value = item[key] ?? '';
      input.addEventListener('input', () => { item[key] = input.value; });
    }
    const label = el('label', '', card); label.textContent = 'Posição da arte';
    const select = el('select', '', label);
    for (const [value, title] of [['top', 'Acima do rosto'], ['bottom', 'Abaixo do rosto']]) {
      const opt = el('option', '', select); opt.value = value; opt.textContent = title;
    }
    select.value = item.posicao || 'top';
    select.addEventListener('change', () => { item.posicao = select.value; });
    workflowButton(card, 'Remover faixa', () => { splitUI.itens.splice(i, 1); drawSplitRows(); });
    if (item.arquivo) workflowButton(card, 'Refazer só esta arte', () => sendSplit(i));
  });
}
async function sendSplit(refazer = null) {
  if (!S.videoId) { toast('Abra um vídeo na Fila primeiro.'); return; }
  try {
    const enquadramento = JSON.parse($('splitFraming').value);
    const itens = splitUI.itens.map((item, i) => ({...item, refazer: i === refazer}));
    if (refazer !== null && itens.some(item => !item.arquivo)) throw new Error('Gere as novas faixas antes de refazer uma arte isolada.');
    await mutateVideo($('splitGenerate'), videoPath(S.videoId) + '/split',
      {familia: $('splitFamily').value, enquadramento, itens}, () => {
        splitUI.video = null;
        openWorkflowTab('fila');
      });
  } catch (e) { toast(e instanceof SyntaxError ? 'Informe o enquadramento medido em JSON válido.' : e.message, 6000); }
}

// ---------- o que esta instalação tem ligado ----------
// Capa e arte da tela dividida dependem de um gerador de imagem externo, que é
// opcional. Sem ele, a tela diz isso no lugar certo e desliga só os dois botões
// — em vez de deixar a pessoa escrever a frase e ver o erro depois.
const recursos = { capa: true, arte: true, motivo_imagem: null, ia: true, voltar: null };
async function carregarRecursos() {
  try { Object.assign(recursos, await videoRequest('/api/recursos')); } catch { /* servidor antigo: segue como estava */ }
  if (recursos.voltar) { $('voltar').href = recursos.voltar; $('voltar').classList.remove('hidden'); }
  if (!recursos.capa) {
    $('coverGo').disabled = true;
    $('coverNote').className = 'recurso-off';
    $('coverNote').textContent = recursos.motivo_imagem;
  }
  if (!recursos.arte) {
    $('splitGenerate').disabled = true;
    $('splitNotice').className = 'recurso-off';
    $('splitNotice').textContent = recursos.motivo_imagem;
  }
}

// ---------- biblioteca de vídeos (mesmo domínio da API) ----------
const library = { videos: [], signature: '', timer: null, busy: false, suggestions: new Map() };
function readWorkflowStyle() {
  try { return JSON.parse(localStorage.getItem(S.videoId ? 'videos_estilo_' + S.videoId : 'videos_estilo') || '{}') || {}; } catch { return {}; }
}
function workflowSettings() {
  return { escala: +$('captionScale').value, subir: +$('captionRise').value,
    headlineEscala: +$('headlineScale').value,
    headlineLinhas: $('headlineLines').value,
    fechoPalavra: $('closingWord').value, fechoAntes: $('closingBefore').value,
    fechoDepois: $('closingAfter').value, trilha: $('soundtrack').value,
    trilhaLigada: $('soundtrackEnabled').checked, picks: S.style };
}
function updateRenderSummary() {
  const g = workflowSettings();
  const preview = $('closingPreview');
  preview.replaceChildren();
  if (g.fechoPalavra.trim()) {
    el('span', '', preview).textContent = g.fechoAntes || 'digita';
    el('strong', '', preview).textContent = g.fechoPalavra.toUpperCase();
    el('span', '', preview).textContent = g.fechoDepois || 'aqui embaixo';
    el('span', '', preview).textContent = '👇';
  } else preview.textContent = 'Sem palavra, o vídeo sai sem fechamento. Escreva uma acima para ver a prévia.';
  const v = library.videos.find(v => v.id === S.videoId);
  $('renderTarget').textContent = v ? `Vídeo aberto: ${v.nome}` : 'Escolha um vídeo pronto na Fila para aplicar o estilo.';
  $('renderVideo').disabled = library.busy || !v || v.status !== 'pronto' || !S.style || S.style.captions === 'none';
  $('renderSummary').textContent = S.style ? `Legenda: ${styleName('captions', S.style.captions)} · tamanho ${g.escala}. Headline: ${S.style.headline === 'none' ? 'sem headline' : g.headlineLinhas.trim() || 'sem headline'}. Edição: ${STYLE_CATALOG.elements.filter(e => S.style.elements[e.id]).map(e => e.name).join(', ') || 'sem extras'}. Trilha: ${g.trilhaLigada ? $('soundtrack').selectedOptions[0].text : 'desligada'}. Fechamento: ${g.fechoPalavra || 'sem fechamento'}.` : '';
}
function saveWorkflowStyle() {
  try { localStorage.setItem(S.videoId ? 'videos_estilo_' + S.videoId : 'videos_estilo', JSON.stringify(workflowSettings())); } catch { /* storage bloqueado */ }
  updateRenderSummary();
}
async function videoRequest(path, options = {}) {
  const r = await fetch(path, { cache: 'no-store', ...options });
  const d = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(typeof d.detail === 'string' ? d.detail : 'Não consegui concluir. Tente novamente.');
  return d;
}
const videoPath = id => '/api/videos/' + encodeURIComponent(id);
function openWorkflowTab(tab) {
  const button = document.querySelector(`[data-tab="${tab}"]`);
  if (button && !button.disabled) button.click();
}
function chooseVideo(id, tab = '1') {
  if (id === S.videoId) { openWorkflowTab(tab); return; }
  if (dirtyCount() && !confirm('Há ajustes não salvos. Trocar de vídeo descarta esses ajustes. Continuar?')) return;
  const params = new URLSearchParams();
  if (document.documentElement.classList.contains('embutido')) params.set('embutido', '1');
  params.set('etapa', tab);
  // Navegar pela rota oficial coloca fd_video antes de carregar qualquer mídia.
  window.location.assign('/editor/' + encodeURIComponent(id) + '?' + params);
}
function renderWorkflowPages() {
  for (const [id, tab] of [['queuePage', 'fila'], ['readyPage', 'prontos'], ['coverPage', 'capas']]) $(id).classList.toggle('hidden', S.tab !== tab);
  if (S.tab === 'capas') carregarCapas();
  document.body.dataset.workflow = String(S.tab);
  $('cutWarning').classList.toggle('hidden', S.tab !== 1);
  const faltando = [!S.editData && 'inserções', !S.captions.length && 'legendas', !S.state.finalVideo && 'vídeo final'].filter(Boolean);
  $('visualNotice').textContent = 'Visual ainda sem ' + faltando.join(', ') + '. Conclua o render da etapa 2; os dados disponíveis já estão na linha do tempo.';
  $('visualNotice').classList.toggle('hidden', S.tab !== 2 || !faltando.length);
  const current = library.videos.find(v => v.id === S.videoId);
  $('cutGate').classList.toggle('hidden', S.tab !== 1 || current?.fase !== 'aprovar_corte');
}
function workflowButton(host, label, action, primary = false) {
  const b = el('button', `btn${primary ? ' primary' : ''}`, host);
  b.textContent = label;
  b.addEventListener('click', action);
  return b;
}
async function mutateVideo(button, path, body, after) {
  if (library.busy) return;
  library.busy = true;
  if (button) button.disabled = true;
  try {
    await videoRequest(path, body === 'DELETE' ? { method: 'DELETE' } : {
      method: 'POST', ...(body ? { headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : {}) });
    await fetchVideos();
    if (after) after();
  } catch (e) { $('libraryMessage').textContent = e.message; toast(e.message, 6000); }
  finally { library.busy = false; if (button) button.disabled = false; updateRenderSummary(); }
}
function decision(host, v, stage, ok) {
  const b = workflowButton(host, ok ? 'Aprovar' : 'Recusar', () => mutateVideo(b, videoPath(v.id) + '/aprovar', { etapa: stage, ok }, () => {
    if (!ok) chooseVideo(v.id, stage === 'final' ? 'style' : '1');
  }), ok);
}
function downloadLink(host, label, url) {
  const a = el('a', 'btn', host); a.textContent = label; a.href = url; a.setAttribute('download', '');
}
function videoCard(host, v, kind, version) {
  const card = el('article', 'glass workflow-card', host);
  const revision = version?.em || v.atualizado_em || v.criado_em || '0';
  // Quando existe a versao com a capa colada na frente, e ELA que toca aqui:
  // e o arquivo que vai pro Instagram. Tocar o antigo fazia ela aprovar uma
  // coisa e publicar outra — "os dois eu aprovo mas nao vi a capa neles".
  const comCapa = !version && kind !== 'cut' && v.com_capa;
  const endpoint = version ? '/versao/' + encodeURIComponent(version.estilo)
    : kind === 'cut' ? '/corte' : comCapa ? '/com-capa' : '/final';
  const url = videoPath(v.id) + endpoint + '?v=' + encodeURIComponent(revision);
  if (['pending', 'approved', 'version', 'cut'].includes(kind)) {
    const player = el('video', 'library-player', card); player.controls = true; player.preload = 'metadata'; player.playsInline = true; player.src = url;
  }
  el('h3', '', card).textContent = v.nome + (version ? ' · ' + styleName('captions', version.estilo) : '');
  if (comCapa) { const p = el('span', 'pill', card); p.textContent = '★ com a capa na frente'; }
  const labels = { na_fila: 'Na fila', processando: 'Montando', pronto: 'Pronto', erro: 'Deu erro' };
  el('span', 'pill', card).textContent = labels[v.status] || v.status;
  el('p', '', card).textContent = v.status === 'erro' ? v.erro || 'Falhou' : v.etapa || (v.status === 'na_fila' ? 'Esperando a vez' : '');
  if (v.dur_final != null) el('p', '', card).textContent = `${fmt(v.dur_original || 0)} → ${fmt(v.dur_final)}${v.cortado_pct != null ? ` · ${v.cortado_pct}% fora` : ''}`;
  const actions = el('div', 'workflow-actions', card);
  if (kind === 'pending') decision(actions, v, 'final', true), decision(actions, v, 'final', false);
  if (kind === 'cut' && v.fase === 'aprovar_corte') decision(actions, v, 'corte', true), decision(actions, v, 'corte', false);
  if (v.status === 'pronto') workflowButton(actions, v.id === S.videoId ? 'Editar vídeo aberto' : 'Abrir no editor', () => chooseVideo(v.id));
  if (['pending', 'approved', 'version', 'cut'].includes(kind)) downloadLink(actions, 'Baixar', url);
  if (kind === 'queue' || kind === 'select') {
    const b = workflowButton(actions, 'Apagar', () => {
      if (!confirm(`Apagar “${v.nome}” e todos os arquivos e versões?`)) return;
      if (v.id === S.videoId && dirtyCount() && !confirm('Este vídeo tem ajustes não salvos. Apagar mesmo assim?')) return;
      mutateVideo(b, videoPath(v.id), 'DELETE', () => {
        if (v.id === S.videoId) window.location.assign('/editor' + (document.documentElement.classList.contains('embutido') ? '?embutido=1' : ''));
      });
    });
    b.disabled = v.status === 'processando';
    if (b.disabled) b.title = 'Espere terminar o processamento para apagar.';
  }
}
function fillVideoList(id, items, kind, empty) {
  const host = $(id); host.replaceChildren();
  if (!items.length) el('p', '', host).textContent = empty;
  for (const v of items) videoCard(host, v, kind);
}
function renderLibrary() {
  const ready = library.videos.filter(v => v.status === 'pronto');
  const queue = library.videos.filter(v => v.status !== 'pronto');
  $('queueCount').textContent = `${queue.length} na fila`;
  fillVideoList('queueList', queue, 'queue', 'Nenhum vídeo na fila. Mande um vídeo acima.');
  fillVideoList('videoList', ready, 'select', 'Os vídeos aparecem aqui quando o corte terminar.');
  fillVideoList('pendingList', ready.filter(v => v.fase === 'aprovar_final'), 'pending', 'Nenhum vídeo esperando aprovação final.');
  fillVideoList('approvedList', ready.filter(v => v.fase === 'aprovado'), 'approved', 'Nenhum vídeo aprovado ainda. O que você aprovar fica guardado para postar.');
  fillVideoList('versionsList', ready, 'cut', 'Nenhum corte pronto. Envie um vídeo na Fila.');
  for (const v of ready) for (const version of v.versoes || []) videoCard($('versionsList'), v, 'version', version);
  const gate = $('cutGate'); gate.replaceChildren();
  const v = ready.find(v => v.id === S.videoId);
  if (v?.fase === 'aprovar_corte') {
    el('p', '', gate).textContent = library.suggestions.get(v.id) || 'A IA propôs este corte.';
    const actions = el('div', 'workflow-actions', gate); decision(actions, v, 'corte', true); decision(actions, v, 'corte', false);
    downloadLink(actions, 'Baixar corte', videoPath(v.id) + '/corte?v=' + encodeURIComponent(v.atualizado_em || '0'));
  }
  renderWorkflowPages(); updateRenderSummary();
}
// ---------- capas ----------
// A referencia da casa e uma IMAGEM, nao um paragrafo: descrever o estilo em
// palavras produziu uma capa com OUTRA pessoa. Por isso "usar como referencia"
// existe — trocar o padrao e trocar o arquivo.
async function carregarCapas() {
  if (!S.videoId) {
    $('coverList').innerHTML = '';
    if (recursos.capa) $('coverNote').textContent = 'Escolha um vídeo na Fila primeiro.';
    return;
  }
  try {
    const d = await videoRequest(videoPath(S.videoId) + '/capas');
    const itens = d.itens || [];
    const familias = d.familias || [];
    // O seletor guarda a escolha dela; se a família sumir, volta pro padrão.
    const sel = $('coverFamily'), antes = sel.value;
    sel.innerHTML = '';
    for (const f of familias) {
      const o = el('option', '', sel);
      o.value = f.nome;
      o.textContent = f.nome + (f.padrao ? ' · padrão' : '');
    }
    sel.value = familias.some(f => f.nome === antes) ? antes : (d.padrao || '');
    $('coverCount').textContent = itens.length ? `${itens.length} feita(s)` : 'nenhuma ainda';
    const host = $('coverList');
    host.innerHTML = '';
    if (!itens.length) {
      const p = el('p', '', host);
      p.textContent = 'Nenhuma capa ainda. Escreva a frase acima e mande fazer.';
    }
    for (const c of itens) {
      const card = el('div', 'glass workflow-card', host);
      const img = el('img', 'cover-thumb', card);
      img.src = videoPath(S.videoId) + '/capa/' + encodeURIComponent(c.nome) + '?v=' + encodeURIComponent(c.em);
      img.alt = 'Capa ' + c.nome;
      const t = el('div', 'cover-name', card);
      t.textContent = c.nome.replace('.png', '').replace('-', ' · ');
      if (c.referencia) {
        const pill = el('span', 'pill', t);
        pill.textContent = '★ referência de ' + c.referencia;
      }
      const acoes = el('div', 'workflow-actions', card);
      const a = el('a', 'btn', acoes);
      a.href = img.src; a.download = c.nome; a.textContent = 'Baixar';
      if (!c.referencia) {
        workflowButton(acoes, 'Usar como referência', () => {
          const nome = prompt('Guardar como referência de qual linguagem?\n\nUse um nome existente pra substituir, ou um novo pra criar outra família.', d.padrao || 'colagem');
          if (!nome) return;
          mutateVideo(null, videoPath(S.videoId) + '/capa/' + encodeURIComponent(c.nome) + '/referencia',
            { familia: nome }, () => { toast('Guardada. As capas em "' + nome + '" seguem esta.', 6000); carregarCapas(); });
        });
      }
      workflowButton(acoes, 'Pôr 1s na frente do vídeo', () =>
        mutateVideo(null, videoPath(S.videoId) + '/capa/' + encodeURIComponent(c.nome) + '/na-frente',
          null, () => { toast('Colando. O vídeo com a capa aparece em Prontos.', 6000); openWorkflowTab('fila'); }), true);
      workflowButton(acoes, 'Apagar', () => {
        if (!confirm('Apagar esta capa?')) return;
        mutateVideo(null, videoPath(S.videoId) + '/capa/' + encodeURIComponent(c.nome), 'DELETE', carregarCapas);
      });
    }
    const ref = $('coverRef');
    ref.innerHTML = '';
    if (!familias.length) {
      el('p', '', ref).textContent = 'Ainda não há referência. Faça uma capa e marque como referência.';
    }
    for (const f of familias) {
      const card = el('div', 'glass workflow-card', ref);
      const img = el('img', 'cover-thumb', card);
      img.src = '/api/capas/referencia/' + encodeURIComponent(f.nome) + '?v=' + encodeURIComponent(f.em);
      img.alt = 'Referência ' + f.nome;
      const t = el('div', 'cover-name', card);
      t.textContent = f.nome;
      if (f.padrao) { const pill = el('span', 'pill', t); pill.textContent = '★ padrão'; }
      const acoes = el('div', 'workflow-actions', card);
      if (!f.padrao) {
        workflowButton(acoes, 'Virar padrão', () =>
          mutateVideo(null, '/api/capas/padrao', { familia: f.nome },
            () => { toast('“' + f.nome + '” virou o padrão.', 5000); carregarCapas(); }));
      }
    }
  } catch (e) {
    if (recursos.capa) $('coverNote').textContent = e.message;
  }
}
...
async function fetchVideos() {
  ...
  try {
    const d = await videoRequest('/api/videos');
    library.videos = d.itens || [];
    $('libraryMessage').textContent = '';
    const signature = JSON.stringify(library.videos) + S.videoId;
    if (signature !== library.signature) {
      library.signature = signature;
      const v = library.videos.find(v => v.id === S.videoId && v.fase === 'aprovar_corte');
      if (v) {
        try { const falas = await videoRequest(videoPath(v.id) + '/falas'); library.suggestions.set(v.id, falas.sugestao?.resumo); }
        catch { library.suggestions.delete(v.id); }
      }
      renderLibrary(); // Só reconstrói players quando a lista muda, preservando reprodução no polling.
    }
  } catch (e) { $('libraryMessage').textContent = 'Não consegui falar com o servidor de vídeo. Fico tentando. ' + e.message; }
  finally { library.timer = setTimeout(fetchVideos, library.videos.some(v => ['na_fila', 'processando'].includes(v.status)) ? 4000 : 30000); }
}
function initWorkflow() {
  const g = readWorkflowStyle();
  const fields = { captionScale: 'escala', captionRise: 'subir', headlineScale: 'headlineEscala', headlineLines: 'headlineLinhas', closingWord: 'fechoPalavra', closingBefore: 'fechoAntes', closingAfter: 'fechoDepois', soundtrack: 'trilha' };
  for (const [id, key] of Object.entries(fields)) if (g[key] != null) $(id).value = g[key];
  if (typeof g.trilhaLigada === 'boolean') $('soundtrackEnabled').checked = g.trilhaLigada;
  else if (Array.isArray(g.ligados)) $('soundtrackEnabled').checked = !!g.ligados[4];
  S.style = { ...defaultStyle(), ...g.picks };
  if (!g.picks && Array.isArray(g.ligados)) ['zoomCuts', 'zoomAuto', 'tracking', 'flashCut'].forEach((key, i) => S.style.elements[key] = !!g.ligados[i]);
  if (!g.picks && Number.isInteger(g.headline)) S.style.headline = ['outline', 'card', 'realce', 'misto'][g.headline] || 'outline';
  $('splitAdd').addEventListener('click', () => { splitUI.itens.push({inicio: '', fim: '', descricao: '', posicao: S.style.edit === 'split2' ? 'bottom' : 'top'}); drawSplitRows(); });
  $('splitGenerate').addEventListener('click', () => sendSplit());
  $('styleSetup').addEventListener('input', saveWorkflowStyle);
  $('styleSetup').addEventListener('click', saveWorkflowStyle);
  $('uploadVideo').addEventListener('change', async () => {
    const input = $('uploadVideo'), file = input.files[0]; if (!file) return;
    input.disabled = true; $('libraryMessage').textContent = 'Enviando…';
    try { const fd = new FormData(); fd.append('arquivo', file); await videoRequest('/api/videos', { method: 'POST', body: fd }); await fetchVideos(); }
    catch (e) { $('libraryMessage').textContent = e.message; }
    finally { input.disabled = false; input.value = ''; }
  });
  $('renderVideo').addEventListener('click', () => {
    const v = library.videos.find(v => v.id === S.videoId);
    if (!v || v.status !== 'pronto' || S.style.captions === 'none') return;
    if (dirtyCount() || S.hasPending || S.savedPending) { toast('Salve e aplique os ajustes do corte antes de renderizar o estilo.', 6000); return; }
    const g = workflowSettings(), elements = S.style.elements;
    const payload = { legenda: S.style.captions, fonte_escala: g.escala, legenda_subir: g.subir };
    for (const [key, api] of [['zoomCuts', 'zoom_cortes'], ['zoomAuto', 'zoom_auto'], ['tracking', 'rosto'], ['flashCut', 'flash']]) if (elements[key]) payload[api] = true;
    if (g.trilhaLigada) payload.trilha = g.trilha;
    if (g.headlineLinhas.trim() && S.style.headline !== 'none') { payload.headline = S.style.headline; payload.headline_escala = g.headlineEscala; payload.linhas = g.headlineLinhas.split('\n').map(l => l.trim()).filter(Boolean).slice(0, 3); }
    if (g.fechoPalavra.trim()) Object.assign(payload, { fechamento: g.fechoPalavra.trim(), fechamento_antes: g.fechoAntes.trim() || 'digita', fechamento_depois: g.fechoDepois.trim() || 'aqui embaixo' });
    mutateVideo($('renderVideo'), videoPath(v.id) + '/legendar', payload, () => openWorkflowTab('fila'));
  });
  $('coverGo').addEventListener('click', () => {
    const frase = $('coverPhrase').value.split('\n').map(l => l.trim()).filter(Boolean).slice(0, 4).join('|');
    if (!frase) { toast('Escreva a frase da capa primeiro.', 5000); return; }
    if (!S.videoId) { toast('Escolha um vídeo na Fila primeiro.', 5000); return; }
    $('coverNote').textContent = 'Na fila. A capa leva alguns minutos — ela é desenhada, não montada.';
    mutateVideo($('coverGo'), videoPath(S.videoId) + '/capa', { frase, familia: $('coverFamily').value }, () => openWorkflowTab('fila'));
  });
  const requested = new URLSearchParams(location.search).get('etapa');
  S.tab = ['1', '2'].includes(requested) ? +requested : ['fila', 'style', 'prontos', 'capas'].includes(requested) ? requested : 'fila';
  openWorkflowTab(String(S.tab));
  updateRenderSummary(); fetchVideos(); carregarRecursos();
}
